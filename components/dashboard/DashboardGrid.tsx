...
/** Fallback: läs från localStorage (används vid saknad inloggning). */
function loadLocalLayouts() {
  if (typeof window === "undefined") return DEFAULT_LAYOUTS;
  try {
    const stored = localStorage.getItem(STORAGE_KEY);
...
export default function DashboardGrid({ username }: { username: string | null }) {
  ...
  // Ladda layout: från server om inloggad, annars localStorage
  useEffect(() => {
    async function init() {
      if (username) {
        try {
          const res = await fetch("/api/layout", { cache: "no-store" });
          if (res.ok) {
            const data = await res.json() as { layouts: typeof DEFAULT_LAYOUTS | null; hidden: string[] };
            if (data.layouts) setLayouts(data.layouts);
            if (data.hidden?.length) setHidden(new Set(data.hidden as WidgetId[]));
          }
        } catch {
          // Fallback till localStorage om nätverksfel
          setLayouts(loadLocalLayouts());
        }
      } else {
        setLayouts(loadLocalLayouts());
  ...
  // Spara med debounce (1.5s) för att undvika för många anrop vid dragging
  const persistPrefs = useCallback(
    (newLayouts: typeof DEFAULT_LAYOUTS, newHidden: Set<WidgetId>) => {
      if (saveTimerRef.current) clearTimeout(saveTimerRef.current);
      saveTimerRef.current = setTimeout(() => {
  ...
  const resetLayout = () => {
    if (saveTimerRef.current) clearTimeout(saveTimerRef.current);
    const newHidden = new Set<WidgetId>();
    setLayouts(DEFAULT_LAYOUTS);
    setHidden(newHidden);
    if (username) {
      fetch("/api/layout", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ layouts: DEFAULT_LAYOUTS, hidden: [] }),
      }).catch(() => {});
    } else {
      localStorage.removeItem(STORAGE_KEY);
    }
  ...
  // Vänta tills layout är laddad för att undvika "flash" av default
  if (!loaded) return null;
